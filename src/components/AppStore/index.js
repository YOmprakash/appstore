...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, searchResult: ''}

  displayApps = () => {
    const {activeTabId} = this.state
    const filterApps = appsList.filter(each => each.category === activeTabId)
  ...
  render() {
    const filterApps = this.displayApps()
    const {activeTabId, searchResult} = this.state
